...
boolean SHOW_SENSOR = true; //DISPLAY SESNORS keypress "s" to toggle
...
int blockSizeWidth = 1;//5
int blockSizeHeight = 2;//10
...
void draw() {
  ...
  if (ACTIVE) {
    ...
    for (int i = 0; i < Z.length; i++) {
      ...
      r = map(constrain(Z[i].magnitude,0,80), 0,80,.1,0.3);
        stroke(r, 1, 1);
     // }
      Z[i].display();
    }
